...
const splitLoopSession = (
    ...
    while (currentVocalTime < duration) {
        ...
        sections.push({
            ...
            type: classifySection(channelData, sampleRate, currentVocalTime, passEndTime),
...
const splitLinearSession = (
        ...
        if (isQuiet && !inSilence) {
            ...
            if (silenceDuration >= minSilenceDuration && currentVocalStart < silenceStart) {
                ...
                sections.push({
                    ...
                    type: classifySection(channelData, sampleRate, currentVocalStart, silenceStart),
                    isBest: true,
                    isFavorited: false
                });
    ...
    if (currentVocalStart < duration) {
        ...
        if (finalEndTime - currentVocalStart > 0.5) {
            sections.push({
                ...
                type: classifySection(channelData, sampleRate, currentVocalStart, finalEndTime),
                isBest: true,
                isFavorited: false
            });
        }
    ...
    if (sections.length === 0) {
        sections.push({
            ...
            startTime: 0,
            endTime: duration,
            type: classifySection(channelData, sampleRate, 0, duration),
            isBest: true,
            isFavorited: false
        });
    }
